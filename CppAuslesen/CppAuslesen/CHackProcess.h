...
public:
	...
	{	 
		...
		do{if (!_strcmpi((char*)__ProcessEntry.szExeFile, __ProcessName))
		{
...
{ 
		...
		while(__RunModule)
		{
			if(!strcmp((char*)lpModuleEntry.szModule, LPSTRModuleName ) )
			{CloseHandle( hSnapShot );
...
void runSetDebugPrivs() 
{
	...
	LookupPrivilegeValue(0, (LPCWSTR)"seDebugPrivilege", &__LUID);
